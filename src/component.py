...
class Component(ComponentBase):
        ...
        for mp in self.marketplace_ids:
            for start_date, end_date in order_segments:
                ...
                if report_id:
                    ...
                    ), 'orders.csv', is_xml=True, primary_keys=['amazon_order_id', 'sku', 'asin'])
            ...
            for message in root.findall('Message'):
                ...
                for item in order.findall('OrderItem'):
                    ...
                    item_details = {
                        ...
                        'quantity': safe_int(get_text_from_node(item, 'Quantity', '0')),
                        'number_of_items': safe_int(get_text_from_node(item, 'NumberOfItems', '0')),
                        'currency': (item_price_node.find('.//Amount').get('currency') 
                                    if item_price_node and item_price_node.find('.//Amount') is not None 
                                    else ''),
                        ...
                        'item_promotion_discount': safe_float(get_text_from_node(promotion_node, 'ItemPromotionDiscount', '0.0')),
                        'ship_promotion_discount': safe_float(get_text_from_node(promotion_node, 'ShipPromotionDiscount', '0.0')),
    ...
    def process_data(self, df, file_name, primary_keys, process_empty=False):
        ...
        if not df.empty or process_empty:
